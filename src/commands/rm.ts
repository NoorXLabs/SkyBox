...
import { getLockStatus, releaseLock } from "../lib/lock.ts";
...
export async function rmCommand(
	...
	// Check lock status and release if owned by us
	const lockSpin = spinner("Checking lock status...");
	try {
		const lockStatus = await getLockStatus(project, config);

		if (lockStatus.locked) {
			if (lockStatus.ownedByMe) {
				lockSpin.text = "Releasing lock...";
				const releaseResult = await releaseLock(project, config);
