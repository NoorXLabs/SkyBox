...
import { pauseSync, terminateSync } from "../lib/mutagen";
import {
  getContainerStatus,
  ContainerStatus,
  stopContainer,
...
export async function downCommand(
  ...
  if (shouldCleanup && containerInfo) {
    ...
    const removeResult = await removeContainer(projectPath, { removeVolumes: true });
    ...
    const { removeLocal } = await inquirer.prompt([
      {
        type: "confirm",
        name: "removeLocal",
        message: "Also remove local project files? (Remote copy will be preserved)",
        default: false,
      },
    ]);
  ...
  if (shouldRemoveLocal) {
    ...
    try {
      ...
      info(`Remote copy preserved at ${config.remote.host}:${config.remote.base_path}/${project}`);
